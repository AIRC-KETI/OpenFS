...
class MediaPipeDatasetConCat(MediaPipeDataset):
    """
        concatenate joint-wise per person
    """
    def __init__(
        self,
        data_path,
    ...
    def __getitem__(self, index):
        ...
        for i, person_id in enumerate(all_ids):
            ...
            # Align the number of frame with zero padding
            max_len = max(len(right), len(left))

            if 0 < len(right) < max_len:
                pad = np.zeros((max_len - len(right), 21, 3), dtype=np.float32)
                right = np.concatenate([right, pad], axis=0)
            elif len(right) == 0:
                right = np.zeros((max_len, 21, 3), dtype=np.float32)

            if 0 < len(left) < max_len:
                pad = np.zeros((max_len - len(left), 21, 3), dtype=np.float32)
                left = np.concatenate([left, pad], axis=0)
            elif len(left) == 0:
                left = np.zeros((max_len, 21, 3), dtype=np.float32)

            if len(right) > 0 or len(left) > 0:
                pose = np.concatenate([right, left], axis=1)
                if self.representation == '2d':
                    pose = pose[..., :2]
                poses_list.append(pose)
                idx_pose = np.arange(len(pose), dtype=np.int64)
                frame_list.append(idx_pose)
                seg = np.full(len(left),  person_cnt, dtype=np.int64)
                seg_list.append(seg)
                person_cnt += 1
...
class MediaPipeDatasetConCatV2(MediaPipeDataset):
    """
        concatenate joint-wise all people
    """
    def __init__(
        self,
        data_path,
    ...
    def __getitem__(self, index):
        ...
        for i, person_id in enumerate(all_ids):
            ...
            if len(right) > 0:
                hand_pose_list.append(right)
            if len(left) > 0:
                hand_pose_list.append(left)

        max_len = max(p.shape[0] for p in hand_pose_list)

        padded = []
        for p in hand_pose_list:
            T, J, d = p.shape
            if T < max_len:
                pad = np.zeros((max_len - T, J, d), dtype=p.dtype)
                p = np.concatenate([p, pad], axis=0)
            padded.append(p)

        pose = np.concatenate(padded, axis=1)

        if self.representation == '2d':
            pose = pose[..., :2]

        T_pose, J_pose, D_pose = pose.shape
        if J_pose < 420:
            pad = np.zeros((T_pose, 420 - J_pose, D_pose), dtype=pose.dtype)
            pose = np.concatenate([pose, pad], axis=1)

        poses_list.append(pose)
        idx_pose = np.arange(len(pose), dtype=np.int64)
        frame_list.append(idx_pose)
        seg = np.full(len(pose),  0, dtype=np.int64)
        seg_list.append(seg)
